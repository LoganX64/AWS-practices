...
Cloud computing simple terms - instead of using/deploying your own physical server, we use someone else server over the internet.
...
we pay monthly rent and landlord handles the maintenance
...
The company owns or leases physical infrastructure and manages it. AWS provides on-premises deployment options using hybrid cloud services like AWS Outposts, which delivers native AWS infrastructure and services directly to our local data center.
...
AWS generally follows a pay-as-you-go model for many services. we use a resource → AWS charges according to the applicable pricing model. pricing may varies based on services
