...
The main idea is to understand where AWS infrastructure exists and how we can design applications to survive failures and serve users efficiently.
...
Suppose our application is running only in one AZ:
...
If that AZ has a problem, our application can become unavailable.
...
Suppose our website's images are stored in an S3 bucket.
...
If we want the application in another Region, we generally need to deploy/replicate the required resources there.
...
1. Latency :
   Choose infrastructure reasonably close to our users.
...
5. Disaster recovery :
   we may choose an additional Region for disaster recovery.
...
Multi-Region means deploying our application or its components across multiple AWS Regions.
